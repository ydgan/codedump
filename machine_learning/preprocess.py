...
for root, dirs, files in os.walk(r'E:\data\machine_learning\dataset'):
    ...
    for file in [x for x in files if x.endswith('.csv')]:
        ...
        mdata['MidPrice'] = ((mdata['AskPrice1'] + mdata['BidPrice1']) / 2).pct_change()
        for i in [1, 2, 3, 4, 5, 6, 7, 8, 9, 'A']:
            ...
            #mdata['Wap%s'%i] = mdata['Wap%s'%i].pct_change()
            ...
            mdata['AskPrice%s'%i] = mdata['AskPrice%s'%i].pct_change()
            mdata['BidPrice%s'%i] = mdata['BidPrice%s'%i].pct_change()
