...
if __name__ == '__main__':
    ...
    fac_path = os.path.join(r'E:\sector_rotation\multi_factors', str(rebal_freq), 'combined')

    start_date = pd.to_datetime('2018-01-01')
    end_date = pd.to_datetime('2023-12-31')

    rep_path = os.path.join(r'E:\sector_rotation\multi_factors', str(rebal_freq), 'backtesting',
                            '%s-%s-Rep.csv'%(start_date.strftime('%Y%m%d'), end_date.strftime('%Y%m%d')))
    ...
    for root, dirs, files in os.walk(fac_path):
        ...
        for file in files:
            ...
            if date in resample_date['rebalance'].values:
                tmp_fact = pd.read_csv(os.path.join(root, file), dtype={'Code':object})
                ...
                tmp_fact = tmp_fact.sort_values('Equal_Weights', ascending=False)
                tmp_fact['score'] = np.linspace(1, 0, tmp_fact.shape[0])

                tmp_sig = tmp_fact.groupby('行业代码')['score'].mean().to_frame().sort_values('score', ascending=False)
    ...
    plot_path = os.path.join(r'E:\sector_rotation\multi_factors', str(rebal_freq), 'backtesting', 'Equal_Weights')
    ...
    csi500_annual_return = empyrical.annual_return(asset_value['CSI500'].pct_change())
