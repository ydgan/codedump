...
def factor_neut(loc, factor, mcap, industry):
    ...
    res_fit = sm.OLS(factor[loc].astype(float), factor['mcap'].astype(float)).fit()
...
if __name__ == '__main__':
    ...
    for root, dirs, files in os.walk(raw_fac_path):
        ...
        #p1 = mp.Pool()
        #for file in files:
            #p1.apply_async(preprocess, (root, file, isST, isNew, mcap, industry, fac_path), error_callback=err_call)
        #p1.close()
        #p1.join()
